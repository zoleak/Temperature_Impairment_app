...
final_continuous_temp_data<-read_csv("final_continuous_temp_check.csv",col_names = T)%>%
  dplyr::rename(stdate = Date)
...
### Read in shapefiles ###
### Weather stations shapefile ###
wx_stations<-st_read(getwd(),layer = "WxStationLocations")
### Temp. impairment HUC14s shapefile ###
temp_impaired_hucs<-st_read(getwd(),layer = "Temp_Impairments")
### Shapefile of NJ's HUC14s ###
NJ_HUCs<-st_read(getwd(),layer = "2014_NJ_Integrated_Report_AU")
imp_temp_HUC<-unique(temp_impaired_hucs$HUC14TXT)
### Shapefile of 2016 monitoring stations ### 
monitoring_stations<-st_read(getwd(),layer = "2016_MonitoringSites_FINAL")
### Shapefile of 3rd order & up streams ###
streams<-st_read(getwd(),layer = "3rdorderupStreams_HUC14_join")%>%
  select(HUC14,W_NAME,STREAMORDE,geometry)
#### Shapefile of BFF stations ###
BFF_stations<-st_read(getwd(),layer = "BFFstation_huc_join")%>%
  select(Site,Location,Lat,Long_,HUC14,geometry)
### Shapefile of test watershed1 for model ###
watershed1_hucs<-st_read(getwd(), layer = "Test_Watershed1")
### Shapefile of test watershed2 for model ###
watershed2_hucs<-st_read(getwd(), layer = "Test_Watershed2")%>%
  filter(!HUC14 %in% "02020007020030")

### Shapefile of pH impairments in test watersheds for model ###
pH_impairments<-st_read(getwd(),layer = "pH_impairments_Test_watershed2")
### Shapefile of test watershed 1 with 7 day average impairments ###
watershed1_daily_max<-st_read(getwd(),layer = "Test_Watershed1_7day_avg")
### Shapefile of test watershed 1 with diurnal impairments ###
watershed1_diurnal<-st_read(getwd(), layer = "Test_Watershed1_Diurnal_impaired_hucs")
### Shapefile of test watershed 2 with 7 day impairements ###
watershed2_daily_max<-st_read(getwd(), layer = "Test_Watershed2_7day_avg")
### Shapefile of test watershed 2 with diurnal impairments ###
watershed2_dirunal_new<-st_read(getwd(),layer = "test2_watershed_diurnal_new")
### Shapefile of USGS Flow Stations ###
USGS_flow_stations<-st_read(getwd(),layer = "USGS_Flow_Stations")
...
watershed1_hucs<-st_transform(watershed1_hucs,crs = "+init=epsg:4326")
watershed2_hucs<-st_transform(watershed2_hucs,crs = "+init=epsg:4326")
pH_impairments<-st_transform(pH_impairments,crs = "+init=epsg:4326")
watershed1_daily_max<-st_transform(watershed1_daily_max,crs = "+init=epsg:4326")
watershed1_diurnal<-st_transform(watershed1_diurnal,crs = "+init=epsg:4326")
watershed2_daily_max<-st_transform(watershed2_daily_max,crs = "+init=epsg:4326")
watershed2_dirunal_new<-st_transform(watershed2_dirunal_new,crs = "+init=epsg:4326")
...
watershed1_hucs<-st_zm(watershed1_hucs, drop = T, what = "ZM")
watershed2_hucs<-st_zm(watershed2_hucs, drop = T, what = "ZM")
pH_impairments<-st_zm(pH_impairments, drop = T, what = "ZM")
watershed1_daily_max<-st_zm(watershed1_daily_max, drop = T, what = "ZM")
watershed1_diurnal<-st_zm(watershed1_diurnal, drop = T, what = "ZM")
watershed2_daily_max<-st_zm(watershed2_daily_max, drop = T, what = "ZM")
###############################################################################
...
###############################################################################
### Make filters for just test watershed 2 ###
hucs_wanted<-as.vector(watershed2_hucs$HUC14)

filtered_temp_data<-temp_data%>%
  filter(HUC14 %in% hucs_wanted)%>%
  filter(!HUC14 %in% "02020007020030")

filtered_cont_data<-final_continuous_temp_data%>%
  filter(HUC14 %in% hucs_wanted)%>%
  filter(!HUC14 %in% "02020007020030")

filtered_cont_data$stdate<-as.Date(filtered_cont_data$stdate)


filtered_streams<-streams%>%
  filter(HUC14 %in% hucs_wanted)

#filtered_streams<-st_transform(filtered_streams, crs="+proj=longlat +datum=WGS84")



filtered_monit_stations<-monitoring_stations%>%
  filter(HUC14 %in% hucs_wanted)

filtered_cont_stations<-BFF_stations%>%
  filter(HUC14 %in% hucs_wanted)

post_2010_locids<-filtered_temp_data%>%
  dplyr::filter(stdate >= '2010-01-14')

post_2010_locids<-unique(post_2010_locids)
post_2010_locids<-gsub("USGS-","",post_2010_locids)
post_2010_locids<-gsub("NJDEP_AMERICORPS-","",post_2010_locids)
post_2010_locids<-gsub("NJDEP_BFBM-","",post_2010_locids)


#post_2010_locids<-filtered_monit_stations%>%
  #dplyr::filter(NewStation %in% test)
...
server <- function(input, output,session) {
  ...
  output$data <- DT::renderDataTable({
    if(input$tables == "Discrete Water Temperature"){
      DT::datatable(filtered_temp_data,filter = 'top',options = list(scrollX = TRUE,
                                                            pageLength = 100))
    }
    
    else if (input$tables == "Continuous Water Temperature"){
      DT::datatable(filtered_cont_data,filter = 'top',options = list(scrollX = TRUE,
                                                                             pageLength = 100))
    }
  })
  ###############################################################################
  ...
  output$temp_map<-renderLeaflet({
    leaflet(options = leafletOptions(minZoom = 7))%>%
      ...
      addPolygons(data = temp_impaired_hucs,color = "#F3161B",weight = 1,smoothFactor = 1,
                  ...
                  group = "Impaired HUC14s (2016)",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  label = ~ paste(HUC14TXT,AU_name),
                  layerId = ~ temp_impaired_hucs$AU_name)%>%
      
      addPolygons(data = watershed1_hucs,color = "#00FFDD",weight = 1,smoothFactor = 1,
                  opacity = 0.5, fillOpacity = 0.6,
                  group = "Test Watershed 1",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  popup = paste("<h6> HUC Name:</h6>",watershed1_hucs$SW_NAME,"\n","<h6>HUC14 #:<h6/>\n",
                                watershed1_hucs$HUC14,sep = ""))%>%
      addPolygons(data = watershed2_hucs,color = "#9C07FF",weight = 1,smoothFactor = 1,
                  opacity = 0.5, fillOpacity = 0.6,
                  group = "Test Watershed 2",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  popup = paste("<h6> HUC Name:</h6>",watershed2_hucs$SW_NAME,"\n","<h6>HUC14 #:<h6/>\n",
                                watershed2_hucs$HUC14,sep = ""))%>%
      addPolygons(data = watershed1_daily_max,color = "#FDC286",weight = 1,smoothFactor = 1,
                  opacity = 0.5, fillOpacity = 0.6,
                  group = "Test Watershed 1 Daily Max Impairments",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  popup = paste("<h6> HUC Name:</h6>",watershed2_hucs$SW_NAME,"\n","<h6>HUC14 #:<h6/>\n",
                                watershed2_hucs$HUC14,sep = ""))%>%
      addPolygons(data = watershed1_diurnal,color = "#358711",weight = 1,smoothFactor = 1,
                  opacity = 0.5, fillOpacity = 0.6,
                  group = "Test Watershed 1 Diurnal Impairments",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  popup = paste("<h6> HUC Name:</h6>",watershed2_hucs$SW_NAME,"\n","<h6>HUC14 #:<h6/>\n",
                                watershed2_hucs$HUC14,sep = ""))%>%
      addPolygons(data = watershed2_daily_max,color = "#0000FF",weight = 1,smoothFactor = 1,
                  opacity = 0.5, fillOpacity = 0.6,
                  group = "Test Watershed 2 Daily Max Impairments",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  popup = paste("<h6> HUC Name:</h6>",watershed2_hucs$SW_NAME,"\n","<h6>HUC14 #:<h6/>\n",
                                watershed2_hucs$HUC14,sep = ""))%>%
      addPolygons(data = watershed2_dirunal_new,color = "#F23500",weight = 1,smoothFactor = 1,
                  opacity = 0.5, fillOpacity = 0.6,
                  group = "Test Watershed 2 Diurnal Impairments",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  popup = paste("<h6> HUC Name:</h6>",watershed2_dirunal_new$SW_NAME,"\n","<h6>HUC14 #:<h6/>\n",
                                watershed2_dirunal_new$HUC14,sep = ""))%>%
      addPolygons(data = pH_impairments,color = "#FFD700",weight = 1,smoothFactor = 1,opacity = 0.5,fillOpacity = 0.6,
                  group = "pH Impairments",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  popup = paste("<h6> HUC Name:</h6>",pH_impairments$SW_NAME,"\n","<h6>HUC14 #:<h6/>\n",
                                pH_impairments$HUC14,sep = ""))%>%
      ...
      addLayersControl(
        ...
        overlayGroups = c("Impaired HUC14s (2016)","HUC14s","Weather Stations","Test Watershed 1",
                          "Test Watershed 1 Daily Max Impairments",
                          "Test Watershed 1 Diurnal Impairments","Test Watershed 2",
                          "Test Watershed 2 Daily Max Impairments",
                          "Test Watershed 2 Diurnal Impairments","pH Impairments"),
        options = layersControlOptions(collapsed = FALSE))#%>%
    #addLegend("bottomright",colors = c("#636060","#F3161B"),opacity = 0.6,
    #labels = c("HUC14s","Impaired HUC14s"))
  })
  ###############################################################################   
  ...
  output$huc<-renderUI({
    if(input$button == "Stations With Discrete"){
      selectizeInput("huc_input",label = strong("Select HUC14:",style = "color:white;font-weight: bold;font-size:1.3em;"),
                     choices = sort(hucs_wanted))
    }
    
    else{
      selectizeInput("huc_input",label = strong("Select HUC14:",style = "color:white;font-weight: bold;font-size:1.3em;"),
                     choices = c("02040105050050","02040105040050","02040105040080"))
    }
  ...
  output$plot1<-renderPlotly({
    ...
    if (input$button == "Stations With Discrete & Continuous"&input$check_air == FALSE){
      ...
      p<-ggplot(data= reac_temp_data(),aes(x=stdate,y=val,group=1,
                                           text = paste("Date:",stdate,
                                                        ...
                                                        "<br>SWQS:",reac_temp_data()$swqs)))+
        geom_point(aes(color = locid),size=1.3)+
        geom_line(data =  continuous_data_reactive(),aes(color = Station),
    ...
    if(input$button == "Stations With Discrete & Continuous"&input$check_air == TRUE){
      p<-ggplot(data= reac_temp_data(),aes(x=stdate,y=val,group=1,
        ...
        geom_point(aes(color = locid),size=1.3)+
        geom_line(data =  continuous_data_reactive(),aes(color = Station,group=1,
                                                         text = paste("Date:",stdate,
                                                                      "<br>Station:",Station,
  ...
  output$map2<-renderLeaflet({
    leaflet(options = leafletOptions(minZoom = 7))%>%
      ...
      addPolygons(data = temp_impaired_hucs,color = "#F3161B",weight = 1,smoothFactor = 1,
                  ...
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
                  popup = paste("HUC14 Name:\n",temp_impaired_hucs$AU_name,"<br>",
                                "HUC14#:\n",temp_impaired_hucs$HUC14TXT,"<br>",sep = ""))%>%
      
      addPolygons(data = watershed2_hucs,color = "#9C07FF",weight = 8,smoothFactor = 1,fillColor = "#00FFFFFF",
                  opacity = 0.5, fillOpacity = 0.6,
                  group = "Test Watershed 2",
                  highlightOptions = highlightOptions(color = "blue",
                                                      weight = 2,bringToFront = TRUE),
      ...
      addMarkers(data = filtered_monit_stations,group = "Discrete Monitoring Stations",
                 label = paste("Station:",filtered_monit_stations$NewStation,sep = ""),
                 clusterOptions = markerClusterOptions())%>%
      
      #addMarkers(data = post_2010_locids,group = "Post 2010 Monitoring Stations",
                # label = paste("Station:",post_2010_locids$NewStation,sep = ""),
                 #clusterOptions = markerClusterOptions())%>%
      #addPolylines(data = filtered_streams,lng = NULL, lat = NULL,
                   #group = "Streams",paste("Stream Name:",filtered_streams$W_NAME,sep = ""))%>%
      
      addMarkers(data = filtered_cont_stations,group = "Continuous Monitoring Stations",
                 label = paste("Station:",filtered_cont_stations$Site,sep = ""),
                 clusterOptions = markerClusterOptions())%>%
      ...
      addMarkers(data = USGS_flow_stations,group = "USGS Flow Stations",
                 label= paste("Site #:",USGS_flow_stations$SITENO,sep = ""),
                 popup = paste("Site Name:\n",USGS_flow_stations$SITENAME,"<br>",
                               "Site #:\n",USGS_flow_stations$SITENO,sep = ""))%>%
      addLayersControl(
        baseGroups = c("Grey", "Satellite"),
        overlayGroups = c("Impaired HUC14s (2016)","Discrete Monitoring Stations",
                          "Continuous Monitoring Stations","Test Watershed 2",
                          "Test Watershed 2 Daily Max Impairments",
                          "Test Watershed 2 Diurnal Impairments",
                          "Streams","USGS Flow Stations"),
        options = layersControlOptions(collapsed = FALSE))
  })
